...
class WikiWeb2M(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.context == 'section_only':
            ...
            max_text_length = self.max_input_length - self.n_visual_tokens #-1 for graph token
            input_ids = self.tokenizer(inputs, max_length=max_text_length, padding="do_not_pad", truncation=True, return_tensors="pt").input_ids[0]
            image_positions.append(input_ids.shape[0] + torch.arange(self.n_visual_tokens))
            input_ids = torch.cat([input_ids, visual_ids], dim=0)
        ...
        if self.context == "all":
            ...
            if self.context != "text_only":
                ...
                if section_image is not None:
                    ...
                    location_images.append(img_location)
            ...
            if self.context != "section_all":
                ...
                added_image_context_ids = []  # 记录所有已添加图片的 context_id
                    ...
                    if self.context != "text_only":
                        if len(neighbor_images) < self.max_image_neighbors:
                            ...
                            if context_image is not None:
                                ...
                                img_graph_index[context_id] = img_location
                                if added_image_context_ids:
                                    last_added_context_id = added_image_context_ids[-1]
                                    img_edge_list.append((img_graph_index[context_id], img_graph_index[last_added_context_id]))
                                added_image_context_ids.append(context_id)  
                ...
                if added_image_context_ids:
                    min_distance = min(abs(cid - section_id) for cid in added_image_context_ids)
                    closest_context_ids = [cid for cid in added_image_context_ids if abs(cid - section_id) == min_distance]
                    for closest_context_id in closest_context_ids:
                        img_edge_list.append((img_graph_index[section_id], img_graph_index[closest_context_id])) 
            # print(img_edge_list)            
            ...
            img_node_num = self.max_image_neighbors
            for node in range(img_node_num):
                img_edge_list.append((node, node))
            img_edge_index = torch.LongTensor(img_edge_list).t().contiguous()
            img_edge_value = torch.ones((img_edge_index.shape[1]))
            img_graph = torch.sparse_coo_tensor(img_edge_index, img_edge_value, [img_node_num, img_node_num]).to_dense()
            img_graph = utils.normalize_graph(img_graph)
            img_graph = (img_graph > 0).to(torch.long)
            ...
            result["text_graph"] = graph
            result["img_graph"] = img_graph
        # print(result.keys())
        return result
