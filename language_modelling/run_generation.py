...
class Arguments:
    ...
    task: Optional[str] = field(
        default='section_all', metadata={"help": "The domain of OAG datasets"}
    )
    ...
    wandb_project: Optional[str] = field(
        default='graph4mm', metadata={"help": "wandb project name"}
    )
    ...
    freeze_lm: Optional[bool] = field(
        default=True, metadata={"help": "evaluate model on validation set."}
    )
...
def main_worker(gpu, world_size, args, log_dir, run):
    ...
    dist.init_process_group(backend='nccl', init_method='tcp://127.0.0.1:1338', world_size=world_size, rank=gpu)
